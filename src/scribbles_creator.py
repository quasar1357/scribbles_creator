...
def create_even_scribbles(ground_truth, max_perc=0.2, margin=0.75,
                          rel_scribble_len=False, scribble_width=1,
                          mode="all", class_dist="balanced", enforce_max_perc=False,
                          print_steps=False):
    '''Generate the scribble annotation for the ground truth using an even distribution of
    pixels among the chosen scribble types (all, both skeletons or individual skeletons and lines).
    This function uses a default scribble_width of 1, a formula to determine the square size and
    a range for pixels inside a square or line of half to double one square side length.
    ...
    Input:
        ...
        max_perc (float): the maximum percentage of pixels that should be picked
                          (from skeletons and lines)
        margin (float): the margin for minimum nr. pixels that should be picked,
                        as a proportion of the pixels given by max_perc (default: 0.75)
        rel_scribble_len (int/bool): length of the single scribbles relative to pixel dimensions,
                                     i.e. the number of scribbles that would fit the image
                                     (empirical default value: 20/(max_perc**0.25))
        ...
        class_dist (str or float): distribution of the classes in the ground truth:
                                   relative (keep % annot for each class),
                                   absolute (same num pix per class), or
                                   balanced (mean of relative and absolute);
                                   can also be given as a float between 0 and 1 for
                                   linear interpolation between absolute and relative (1=absolute)
        enforce_max_perc (bool): whether to enforce the maximum percentage of pixels in
                                 the scribble annotation (in case it has to be surpassed,
                                 e.g. due to the scribble width)
    ...
    # The square size is calculated by nr of pix that would fit into the dimensions of the image
    ...
    if print_steps:
        print(f"\nmax. perc.: {max_perc}, margin: {margin}, " +
              f"rel_scribble_len: {rel_scribble_len:.2f}, width: {scribble_width}, mode: {mode}, " +
                          f"class_dist: {class_dist}, enforce_max_perc: {enforce_max_perc}, " +
                          f"print_steps: {print_steps}\n")

    # Generate the scribble annotation for the ground truth
    scribbles = create_scribbles(
        ground_truth, scribble_width=scribble_width,
        sk_max_perc=max_perc_per_type, sk_margin=margin, sq_size=sq_size, sq_pix_range=False,
        lines_max_perc=max_perc_per_type, lines_margin=margin, line_pix_range=False,
        mode=mode, class_dist=class_dist, enforce_max_perc=enforce_max_perc,
...
def create_scribbles(ground_truth, scribble_width=1,
                     sk_max_perc=0.05, sk_margin=0.75, sq_size=20, sq_pix_range=False,
                     lines_max_perc=0.05, lines_margin=0.75, line_pix_range=False,
                     mode="all", class_dist="balanced", enforce_max_perc=False,
                     print_steps=False):
    '''
    Generate the scribble annotation for the ground truth.
    Input:
        ground_truth (numpy array): the fully annotated image
        scribble_width (int): width of the individual scribbles
        sk_max_perc (float): maximum percentage of pixels of the ground truth that
                             should be picked from each of the skeletons
        sk_margin (float): for the skeletons - margin for minimum nr. pixels that should be picked,
                           as a proportion of the pixels given by max_perc (default: 0.75)
        sq_size (int): size of the squares (side length)
        sq_pix_range (int): range that the number of pixels in a square shall be in
        lines_max_perc (float): maximum percentage of pixels of the ground truth that
                                should be created by drawing lines between skeleton and edge
        lines_margin (float): for the lines - margin for minimum nr. pixels that should be picked,
                              as a proportion of the pixels given by max_perc (default: 0.75)
        line_pix_range (int): range that the number of pixels for a line shall be in
        mode (str): scribble types to use (lines, prim_sk, sec_sk, both_sk, all)
        class_dist (str or float): distribution of the classes in the ground truth:
                                   relative (keep % annot for each class),
                                   absolute (same num pix per class), or
                                   balanced (mean of relative and absolute);
    ...
    for class_val in class_values:
        ...
        # abs_factor is the factor to turn relative (same perc) into absolute (same nr) distribution
        abs_factor = img_pix / (num_classes * class_pix)
        ...
        # We also allow for a linear interpolation between the absolute and relative distribution;
        # this is done if class_dist is given as a float between 0 and 1
        elif 0 <= class_dist <= 1:
            class_sk_max_perc = ((1 - class_dist) * sk_max_perc +
                                 class_dist * sk_max_perc * abs_factor)
            class_lines_max_perc = ((1 - class_dist) * lines_max_perc +
                                    class_dist * lines_max_perc * abs_factor)
        else:
            raise ValueError(f"Invalid class distribution: {class_dist}. Choose from " +
                              "'relative', 'absolute', 'balanced'; or pass the 'absolute-ness' " +
                              "as a float between 0 and 1.")

        # Calculate total maximum percentage of pixels for the class according to the mode chosen
        ...
        else: raise ValueError(f"Invalid mode: {mode}. Choose from " +
                                "'prim_sk', 'sec_sk', 'both_sk', 'lines', 'all'.")
        if print_steps:
            print(f"CLASS {class_val}, max. pixel: {tot_max_perc:.3f}% = " +
                  f"{int(class_pix*tot_max_perc/100)} pixels")

        # Generate the scribble annotation for the class
        # NOTE: We are not enforcing the max. pixels for all types individually,
        # since we do it in the end for the whole class (avoiding multiple very small scribbles)
        class_scribble_annotation = scribble_class(
            ground_truth=ground_truth, class_val=class_val, scribble_width=scribble_width,
            sk_max_perc=class_sk_max_perc, sk_margin=sk_margin, sq_size=sq_size, sq_pix_range=sq_pix_range,
            lines_max_perc=class_lines_max_perc, lines_margin=lines_margin, line_pix_range=line_pix_range,
            mode=mode, enforce_max_perc=False, print_steps=print_steps)
        ...
        if enforce_max_perc:
            ...
            if print_steps and class_pix_annot_after != class_pix_annot_before:
                print(f"   Enforcing the max. percentage ({tot_max_perc:.3f}%) of " +
                      f"pixels in the scribble annotation of the entire CLASS {class_val}")
                print(f"      Pixels before: {class_pix_annot_before} = " +
                      f"{class_pix_annot_before/class_pix*100:.3f}% | " +
                      f"new pixels: {class_pix_annot_after} = " +
                      f"{class_pix_annot_after/class_pix*100:.3f}%")
                if class_max_pix < 1:
                    print("      WARNING: The theoretical maximum number of " +
                         f"pixels for the CLASS {class_val} ({class_max_pix:.2f}) is " +
                          "below 1. Instead, 1 pixel was kept.")
            class_pix_annot = class_pix_annot_after

        if print_steps:
            print(f"CLASS {class_val} pixels: {class_pix_annot} = " +
                  f"{class_pix_annot/class_pix*100:.3f}% \n")

        # Add the scribble annotation of this class to the full scribble
        # (which is valid, because there is no overlap between the classes)
        scribble_annotation += class_scribble_annotation.astype(np.uint8)
    return scribble_annotation

def scribble_class(ground_truth, class_val, scribble_width=1,
                   sk_max_perc=0.05, sk_margin=0.75, sq_size=20, sq_pix_range=False,
                   lines_max_perc=0.05, lines_margin=0.75, line_pix_range=False,
                   mode="all", enforce_max_perc=False, print_steps=False):
    '''
    ...
    Input:
        ...
        scribble_width (int): the width of the individual scribbles
        sk_max_perc (float): the maximum percentage of pixels of
                             the ground truth that should be picked (from skeletons)
        sk_margin (float): for the skeleton - the margin for minimum nr. pixels that
                           should be picked, as a proportion of
                           the pixels given by max_perc (default: 0.75)
        ...
        lines_max_perc (float): the maximum percentage of pixels of
                                the ground truth that should be created by drawing lines
        lines_margin (float): for the lines - the margin for minimum nr. pixels that
                              should be picked, as a proportion of
                              the pixels given by max_perc (default: 0.75)
    ...
    if mode in ("prim_sk", "sec_sk", "both_sk", "all"):
        # Calculate how many TOTAL pixels of each skeleton are allowed in
        # this class given the percentage
        ...
        # Ensure that the TOTAL maximum number of pixels is at least scribble_width**2
        # (avoiding empty scribble annotations, i.e. allowing for at least a "point scribble")
        if sk_max_pix < scribble_width**2:
            print("   WARNING: The theoretical maximum number of pixels for the SQUARES " +
                 f"({sk_max_pix:.2f}) is below scribble_width**2 ({scribble_width**2}). " +
                 f"Instead, {scribble_width**2} pixel(s) is/are sampled.")
        ...
        # Adjust the range to scribble_width (if the scribble is wider,
        # the range needs to be larger to have similar lengths of the scribbles)
        ...
        if print_steps:
            print(f"   sk_max_pix: {sk_max_pix:.2f}, " +
                     f"sq_size: {sq_size}, " +
                     f"sq_pix_range: {sq_pix_range}")
        # If the primary skeleton is needed, pick squares of it
        if mode in ("prim_sk", "both_sk", "all"):
            prim_sk_squares = pick_sk_squares_optim(prim_sk, gt_class_mask, sk_max_pix=sk_max_pix,
                                                    sk_margin=sk_margin, sq_size=sq_size,
                                                    sq_pix_range=sq_pix_range,
            ...
            if print_steps:
                print(f"      prim_sk_squares pix: {np.sum(prim_sk_squares)} = " +
                      f"{np.sum(prim_sk_squares)/np.sum(gt_class_mask)*100:.3f}%")
            ...
            if enforce_max_perc:
                ...
                if print_steps and prim_sk_pix_after != prim_sk_pix_before:
                    print(f"      -> Enforcing the max. percentage ({sk_max_perc:.3f}%) of " +
                           "pixels in the PRIMARY skeleton scribbles - new nr. pix: " +
                          f"{prim_sk_pix_after} = " +
                          f"{prim_sk_pix_after/np.sum(gt_class_mask)*100:.3f}%")
                    if sk_max_pix_orig < 1:
                        print("      WARNING: The theoretical maximum number of " +
                             f"pixels for the SKELETON SQUARES ({sk_max_pix_orig:.2f}) " +
                              "is below 1. Instead, 1 pixel was kept.")
        # If the secondary skeleton is needed, pick squares of it
        if mode in ("sec_sk", "both_sk", "all"):
            sec_sk_squares = pick_sk_squares_optim(sec_sk, gt_class_mask,
                                                   sk_max_pix=sk_max_pix, sk_margin=sk_margin,
                                                   sq_size=sq_size, sq_pix_range=sq_pix_range,
                                                   scribble_width=scribble_width,
                                                   print_steps=print_steps)
            # Ensure that the scribble is within the ground truth mask
            sec_sk_squares = np.logical_and(sec_sk_squares, gt_class_mask)
            if print_steps:
                print(f"      sec_sk_squares pix: {np.sum(sec_sk_squares)} = " +
                      f"{np.sum(sec_sk_squares)/np.sum(gt_class_mask)*100:.3f}%")
            # If enforce_max_perc option is turned on, remove pixels such that max_perc is ensured
            if enforce_max_perc:
                ...
                if print_steps and sec_sk_pix_after != sec_sk_pix_before:
                    print(f"      -> Enforcing the max. percentage ({sk_max_perc:.3f}%) of " +
                           "pixels in the SECONDARY skeleton scribbles - " +
                          f"new nr. pix: {sec_sk_pix_after} = " +
                          f"{sec_sk_pix_after/np.sum(gt_class_mask)*100:.3f}%")
                    if sk_max_pix_orig < 1:
                        print("      WARNING: The theoretical maximum number of " +
                             f"pixels for the SKELETON SQUARES ({sk_max_pix_orig:.2f}) " +
                              "is below 1. Instead, 1 pixel was kept.")
    ...
    # PICK LINES
    # If lines are needed, create and pick them (lines leading from the primary skeleton to
    # the closest edge of the mask)
    if mode in ("lines", "all"):
        ...
        if lines_max_pix < scribble_width**2:
            print("   WARNING: The theoretical maximum number of pixels for the LINES " +
                 f"({lines_max_pix:.2f}) is below scribble_width**2 ({scribble_width**2}). " +
                 f"Instead, {scribble_width**2} pixel(s) is/are sampled.")
        ...
        # Adjust the range to the scribble_width
        # (if scribbles are wider, ranges need to be higher to have similar lengths of scribbles)
        line_pix_min = int(line_pix_min * scribble_width)
        line_pix_max = int(line_pix_max * scribble_width)
        ...
        lines = create_lines_optim(prim_sk, gt_class_mask, lines_max_pix, lines_margin,
                                   line_pix_range, scribble_width, print_steps=print_steps)
        # Ensure that the scribble is within the ground truth mask
        lines = np.logical_and(lines, gt_class_mask)
        if print_steps:
            print(f"      lines pix: {np.sum(lines)} = " +
                  f"{np.sum(lines)/np.sum(gt_class_mask)*100:.3f}%")
        ...
        if enforce_max_perc:
            ...
            if print_steps and lines_pix_after != lines_pix_before:
                print(f"      -> Enforcing the max. percentage ({lines_max_perc:.3f}%) of " +
                      f"pixels in the LINES scribbles - new nr. pix: {lines_pix_after} = " +
                      f"{lines_pix_after/np.sum(gt_class_mask)*100:.3f}%")
                if lines_max_pix_orig < 1:
                    print("      WARNING: The theoretical maximum number of pixels " +
                         f"for the LINES ({lines_max_pix_orig:.2f}) is below 1. " +
                          "Instead, 1 pixel was kept.")
    if mode == "all":
...
def double_sk_class(gt_mask, closing_prim=0, closing_sec=0):
    ...
    Input:
        ...
        closing_prim (int): the size of the structuring element for the closing operation on
                            the primary skeleton
        closing_sec (int): the size of the structuring element for the closing operation on
    ...
    # Create a dilated version of the primary skeleton for generating the secondary skeleton
    # (otherwise it interprets sees gaps in the primary skeleton)
...
def reduce_scribble(scribbles, max_pix):
    ...
    # If the maximum number of pixels is below 1, pick 1 pixel instead
    # (avoiding empty scribble annotations)
    max_pix = max(1, max_pix)
    # If too many pixels are present in the scribble, raise a warning and
    # pick the requested number of pixels
...
def pick_sk_squares(sk, gt_mask, sk_max_pix=20, sq_size=20, sq_pix_range=(10,40), scribble_width=1,
                    print_details=False):
    '''
    ...
    overshoots = 0
    if print_details:
        print("--- Sampling squares: pix_in_sk", pix_in_sk,
                                    "indx_step", idx_step,
                                    "num_coords", num_coords)
    # Loop until the total number of pixels in all squares approaches the threshold or
    # the end of all pixels in the skeleton is reached
    while overshoots < 100 and idx < num_coords:
        ...
        if print_details:
            print(f"---    current_coordinate: {current_coordinate} | " +
                  f"added_pix (before): {added_pix}, pix_in_square: {pix_in_sq}, " +
                  f"new pix: {future_total-added_pix}, future_total: {future_total}")
        # If the square would push the total number of pixels in all squares above the maximum,
        # skip it and count the overshoot
        ...
        if pix_in_sq < sq_pix_range[0] or pix_in_sq > sq_pix_range[1]:
            ...
            continue
        # If the square is valid, add it to the mask of all squares and
        # update the total number of pixels
        all_squares = future_all_squares
        added_pix = future_total
        # If we have reached the goal maximum number of pixels, we can stop the loop, since
...
def pick_sk_squares_optim(sk, gt_mask, sk_max_pix=20, sk_margin=0.75,
                          sq_size=20, sq_pix_range=(10,0), scribble_width=1,
                          print_steps=False):
    '''
    Pick random squares from the skeleton. Use the base function and
    adjust the parameters while too little squares were added.
    Input:
        sk (numpy array): the skeleton to sample squares from
        sk_max_pix (int): the approximate number of pixels that should be picked
        sk_margin (float): the margin for minimum nr. pixels that should be picked,
                           as a proportion of the pixels given by max_perc (default: 0.75)
        sq_size (int): the size of the squares (side length)
        sq_pix_range (int): the range that the number of pixels in a square shall be in
        scribble_width (int): the width of the individual scribbles
    ...
    '''
    # For checking how many pixels can be sampled max.: Create a dilated version of
    # the skeleton (same as the one that the squares are picked from)
    sk_dilated = binary_dilation(sk, square(scribble_width))
    # Ensure all pixels of the dilated skeleton are within the mask
    sk_dilated = np.logical_and(sk_dilated, gt_mask)
    num_pix_dil = np.sum(sk_dilated)
    # If there are too little pixels in the dilated skeleton,
    # raise a warning and return the entire skeleton
    if num_pix_dil <= sk_max_pix:
        ...
        added_pix = np.sum(squares)
    # If not enough squares were added, try again with smaller squares and
    ...
    while np.all(
        ...
        new_squares = pick_sk_squares(sk_left, gt_mask, sk_max_pix_left,
                                      sq_size, sq_pix_range, scribble_width)
        squares = np.logical_or(squares, new_squares)
        added_pix = np.sum(squares)
    # If we have finished looping because we reached the limit of what we can sample,
    # raise a warning if too little squares were added or an error if no squares were added
    if added_pix == 0:
        print("      ERROR: No squares were added!")
    elif added_pix < max(1, np.floor(sk_max_pix * sk_margin)):
        print(f"      WARNING: It was not possible to sample {sk_margin * 100}% of the " +
              f"requested pixels. Only {added_pix} pixels in squares were added!")
    return squares
...
def get_square(mask, coord, sq_size=20):
    ...
    inc = int(np.ceil(sq_size/2)) # Here, idx can exceed mask as slicing will stop at mask's end
...
def create_lines(sk, gt_mask, lines_max_pix=20, line_pix_range=(10, 40), scribble_width=1,
                 line_crop=2, print_details=False):
    '''
    ...
    Input:
        ...
        scribble_width (int): the width of the individual scribbles
        line_crop (int): the crop of the line (on both sides; i.e. distance to
                         edge and and skeleton will each be half of the crop)
        print_details (bool): whether to print the details of the function
    Output:
    ...
    # Make sure to take at least a certain amount of
    # positions on the skeleton (to avoid too few lines)
    ...
    tried_dilated_lines = []
    if print_details:
        print("--- Sampling lines: pix_in_sk", pix_in_sk, "indx_step", idx_step,
              "num_coords", num_coords, "line_crop", line_crop)
    # Loop until the pixels in all lines approach the threshold (keeps overshooting)
    # or the end of all pixels in the skeleton is reached
    while overshoots < 100 and idx < num_coords:
        ...
        dilated_line = binary_dilation(single_pix_line, square(scribble_width))
        # Ensure all pixels of the (dilated) line are within the mask
        dilated_line = np.logical_and(dilated_line, gt_mask)
        tried_dilated_lines.append(dilated_line)
        pix_in_line = np.sum(dilated_line)
        future_all_lines = np.logical_or(all_lines, dilated_line)
        future_total = np.sum(future_all_lines)
        if print_details:
            line_coords = ((np.argwhere(single_pix_line)[0], np.argwhere(single_pix_line)[-1])
                           if np.sum(single_pix_line) > 0 else 'empty')
            print(f"---    current_coordinate: {current_coordinate}, " +
                  f"line {line_coords} | " +
                  f"added_pix (before): {added_pix}, " +
                  f"pix_in_line: {pix_in_line}, " +
                  f"new pix: {future_total-added_pix}, " +
                  f"future_total: {future_total}")
        # If the line would push the total number of pixels on lines above the maximum,
        # skip it and count the overshoot
        ...
        # and update the total number of pixels
        all_lines = future_all_lines
        added_pix = future_total
        # If we have reached the goal maximum number of pixels,
        # we can stop the loop, since no improvement is possible
    ...
    if print_details:
        ...
        print(f"--- Done sampling lines: avg_len_tried {avg_len_tried:.2f}, " +
              f"min_len_tried {min_len_tried}, max_len_tried {max_len_tried}, " +
              f"avg_pix_tried {avg_pix_tried:.2f}, " +
              f"min_pix_tried {min_pix_tried}, max_pix_tried {max_pix_tried}")
    return all_lines, tried_single_pix_lines, tried_dilated_lines

def create_lines_optim(sk, gt_mask, lines_max_pix=20, lines_margin=0.75, line_pix_range=(10, 40),
                       scribble_width=1, init_line_crop=2,
                       print_steps=False):
    '''
    Create lines leading from a skeleton to the edge of the mask.
    Use the base function and adjust the parameters if no lines were added.
    Input:
        sk (numpy array): the skeleton mask
        gt_mask (numpy array): the ground truth mask
        lines_max_pix (int): the maximum number of pixels that should be picked with all lines
        lines_margin (float): the margin for minimum nr. pixels that should be picked, 
                              as a proportion of the pixels given by max_perc (default: 0.75)
        line_pix_range (int): the range that the number of pixels for a single line shall be in
        scribble_width (int): the width of the individual scribbles
        init_line_crop (int): the crop of the line to start with (on both sides; i.e. distance to
                              edge and and skeleton will each be half of the crop),
                              may be changed during optimization
        print_steps (bool): whether to print the steps of the function
    Output:
        lines (numpy array): the mask of all lines
    '''
    line_crop = init_line_crop
    lines, tried_single_pix_lines, tried_dilated_lines = create_lines(sk, gt_mask,
                                                                      lines_max_pix,
                                                                      line_pix_range,
                                                                      scribble_width,
                                                                      line_crop)
    ...
    while tot_added_pix < max(1, np.floor(lines_max_pix * lines_margin)):
        ...
        # NOTE: technically the line could be even shorter, since the dilation also adds pixels
        # on each end, but this simple approach is chosen since it is more conservative
        ...
        if avg_pix_tried > upper_bound and avg_len_tried > 0:
            ...
            # If the minimum is already below the upper bound, crop the lines towards the average,
            # to allow other lines to also fall below the upper bound
            else:
                crop_increase = (avg_len_tried - needed_line_len) * 0.75
            # Again, rather make the crop steps a bit smaller than absolutely necessary
            # (down to the next int), to be conservative
        ...
        # If lines are too short - before or after trying with cropping - allow for shorter lines
        # (we cannot make them longer than they are)
        # Don't reduce the minimum below 1
            ...
            line_crop = init_line_crop
            # If all pixels (i.e., the maximum tried) are below the minimum,
            # directly set the allowed minimum to the maximum tried
        ...
        new_lines, tried_single_pix_lines, tried_dilated_lines = create_lines(sk, gt_mask,
                                                                              lines_max_pix_left,
                                                                              line_pix_range,
                                                                              scribble_width,
                                                                              line_crop=line_crop)
    ...
    # If we have finished looping because we reached the limit of what we can sample,
    # raise a warning if too little lines were added or an error if no lines were added
    if tot_added_pix == 0:
        print("      ERROR: No lines were added!")
    elif tot_added_pix < max(1, np.floor(lines_max_pix * lines_margin)):
        print(f"      WARNING: It was not possible to sample {lines_margin * 100}% of the " +
              f"requested pixels. Only {tot_added_pix} pixels in lines were added!")
...
def get_line(start_coord, gt_mask, line_crop=2):
    '''
    Take a point on the skeleton (= True in the mask) and draw a line to
    the nearest edge point of the ground truth mask.
    Input:
        ...
        line_crop (int): the crop of the line (on both sides; i.e. distance to
                         edge and and skeleton will each be half of the crop)
    Output:
        final_path_mask (numpy array): the mask of the shortest path,
                                       optionally cropped on both sides
    '''
    shortest_path_mask = point_to_edge(start_coord, gt_mask)
...
def point_to_edge(start_point, segmentation_mask):
    ...
    row_ids, col_ids = line(start_point[0], start_point[1],
